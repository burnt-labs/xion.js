...
export function Providers({ children }: { children: React.ReactNode }) {
  ...
  const createWalletRef = useRef<((params: any) => Promise<string>) | null>(
    null,
  );
        ...
        callbacks={{
          ...
          onAuthenticationSuccess: async () => {
            ...
            while (attempts < maxAttempts) {
              wallets = walletsGetterRef.current
                ? walletsGetterRef.current()
                : [];
              if (wallets.length > 0) {
                break;
              }
            ...
            if (abstraxionLoginRef.current) {
              try {
                ...
                console.error(
                  "[Turnkey] Error calling Abstraxion login:",
                  error,
                );
              }
            }
