...
import { getTestConfig, TEST_MNEMONIC, EXPECTED_VALUES } from "../fixtures";
...
export function registerSignerAuthIntegrationTests(
  selectedSuites: readonly SignerAuthSuite[],
) {
  const enabledSuites = new Set(selectedSuites);
  ...
  describe("Signer Authentication Flow - Integration", () => {
    ...
    describe.runIf(enabledSuites.has("secp256k1"))(
      "Secp256K1 Connector Flow",
      () => {
        it("should connect with Secp256K1 connector", async () => {
        ...
        it("should handle account discovery for existing account", async () => {
    ...
    describe.runIf(enabledSuites.has("ethwallet"))(
      "EthWallet Connector Flow",
      () => {
        it("should connect with EthWallet connector", async () => {
          const mnemonic = generateTestMnemonic();
        ...
        it("should handle address formatting for EthWallet", async () => {
          const signerAuth: SignerAuthentication = {
            type: "signer",
            aaApiUrl: config.aaApiUrl,
            smartAccountContract: {
    ...
    describe.runIf(enabledSuites.has("switching"))(
      "Connector Switching",
      () => {
        it("should switch between different connectors", async () => {
          const mnemonic = generateTestMnemonic();
    ...
    describe.runIf(enabledSuites.has("restoration"))(
      "Session Restoration",
      () => {
        it("should restore session from storage", async () => {
          ...
          expect(["idle", "connecting", "connected"]).toContain(state2.status);
    ...
    describe.runIf(enabledSuites.has("state"))("State Management", () => {
      ...
      it("should allow disconnection and state reset", async () => {
        const signerAuth: SignerAuthentication = {
          type: "signer",
          aaApiUrl: config.aaApiUrl,
          smartAccountContract: {
    ...
    describe.runIf(enabledSuites.has("errors"))("Error Handling", () => {
      it("should handle invalid signer config", async () => {
        const signerAuth: SignerAuthentication = {
          type: "signer",
          aaApiUrl: config.aaApiUrl,
          smartAccountContract: {
    ...
    describe.runIf(enabledSuites.has("accounts"))(
      "Account Creation vs Discovery",
      () => {
        it("should create new account for new authenticator", async () => {
          const mnemonic = generateTestMnemonic();
          ...
          try {
            ...
            console.log(
              "⚠️  Account creation attempted (signature validation expected)",
            );
          }
        }, 120000);

        it("should discover existing account for known authenticator", async () => {
        ...
        it("should use correct authenticator type for account creation", async () => {
          const mnemonic = generateTestMnemonic();
          ...
          const secp256k1Auth: SignerAuthentication = {
            ...
            async getSignerConfig() {
              ...
              expect(result.authenticatorType).toBe(
    ...
    describe.runIf(enabledSuites.has("grants"))(
      ...
      () => {
        ...
        it("should validate granter address matches treasury", async () => {
          const signerAuth: SignerAuthentication = {
            type: "signer",
            aaApiUrl: config.aaApiUrl,
            smartAccountContract: {
          ...
          try {
            ...
            if (state.status === "connected" && state.account?.granterAddress) {
              ...
              expect(isValidXionAddress(state.account.granterAddress)).toBe(
                true,
              );
    ...
    describe.runIf(enabledSuites.has("multiple"))(
      "Multiple Connector Types",
      () => {
        it("should handle switching from Secp256K1 to EthWallet", async () => {
          const mnemonic = generateTestMnemonic();
        ...
        it("should handle concurrent connector instances", async () => {
    ...
    describe.runIf(enabledSuites.has("signatures"))(
      "Authenticator Signature Formatting",
      () => {
        ...
        it("should validate EthWallet signature format", async () => {
          const mnemonic = generateTestMnemonic();
