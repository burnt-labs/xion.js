...
export function registerFeeGrantingIntegrationTests(
  selectedSuites: readonly FeeGrantingSuite[],
) {
  ...
  describe("Fee Granting Integration Tests", () => {
    ...
    describe.runIf(enabledSuites.has("treasury"))(
      "Treasury-Based Fee Granting",
      () => {
        it(
    ...
    describe.runIf(enabledSuites.has("calculation"))("Fee Calculation", () => {
      it(
        ...
        async () => {
          ...
          expect(Number(result.gasUsed)).toBeLessThanOrEqual(
            gasEstimation * 1.5,
          );
    ...
    describe.runIf(enabledSuites.has("grants"))("Grant Configuration", () => {
      it(
