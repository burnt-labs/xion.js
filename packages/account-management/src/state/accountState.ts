...
export type AccountState =
  ...
  | { status: 'connected'; account: AccountInfo; signingClient: GranteeSignerClient }
...
export type AccountStateAction =
  ...
  | { type: 'SET_CONNECTED'; account: AccountInfo; signingClient: GranteeSignerClient }
...
export const AccountStateGuards = {
  ...
  isRedirecting: (state: AccountState): state is { status: 'redirecting'; dashboardUrl: string } =>
    state.status === 'redirecting',

  /** Check if state is connecting */
  isConnecting: (state: AccountState): state is { status: 'connecting'; connectorId?: string } =>
    state.status === 'connecting',

  /** Check if state is configuring permissions for the session */
  isConfiguringPermissions: (state: AccountState): state is { status: 'configuring-permissions'; smartAccountAddress: string } =>
    state.status === 'configuring-permissions',

  /** Check if state is connected (ready to use) */
  isConnected: (state: AccountState): state is { status: 'connected'; account: AccountInfo; signingClient: GranteeSignerClient } =>
...
export function accountStateReducer(
  ...
  switch (action.type) {
    ...
    case 'START_CONFIGURING_PERMISSIONS':
      return { status: 'configuring-permissions', smartAccountAddress: action.smartAccountAddress };
