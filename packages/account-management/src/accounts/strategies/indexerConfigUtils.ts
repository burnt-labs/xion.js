...
import type { UserIndexerConfig, AccountIndexerConfig } from '../../types/indexer';
...
export function convertIndexerConfig(
  ...
  if (indexerConfig.type === 'subquery') {
    if (!smartAccountContract?.codeId) {
      throw new Error('Code ID is required when using Subquery indexer. Provide smartAccountContract with codeId.');
    }
    return {
  ...
  return {
    type: 'numia' as const,
    url: indexerConfig.url,
    authToken: 'authToken' in indexerConfig ? indexerConfig.authToken : undefined,
  };
}
