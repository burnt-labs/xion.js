...
export class AbstraxionAuth {
	...
	callbackUrl?: string;
	treasury?: string;
	private autoLogoutOnGrantChange = true;
	 ...
	 * @param {boolean} autoLogoutOnGrantChange - whether to automatically logout when grant changes are detected (defaults to true)
	 */
	configureAbstraxionInstance(
		...
		this.autoLogoutOnGrantChange = autoLogoutOnGrantChange ?? true;
	}
	 ...
	 * Also checks if the grant is still valid by verifying the expiration and configuration changes.
	 * If valid, sets the abstract account and triggers authentication state change.
	 * If expired or configuration changed, clears local state and prompts reauthorization.
	 *
	...
	async authenticate(): Promise<void> {
		try {
			...
			// Reset grants changed flag at the start of authentication
			this.grantsChanged = false;

			// Get current grants and perform comprehensive validation
			const grantsResponse = await fetchChainGrantsABCI(
				keypairAddress,
				granter,
				this.rpcUrl!,
			);
			...
			// Check expiration for each grant
			const currentTime = new Date().toISOString();
			const validGrant = grantsResponse.grants.some((grant) => {
				const { expiration } = grant;
				return !expiration || expiration > currentTime;
			});

			if (!validGrant) {
				throw new Error("Grants have expired.");
			}

			// Perform comprehensive grant validation to detect configuration changes
			let grantsMatch: boolean;
			if (this.treasury) {
				grantsMatch = await this.compareGrantsToTreasury(grantsResponse);
			} else {
				grantsMatch = this.compareGrantsToLegacyConfig(grantsResponse);
			}

			// Handle grant configuration changes
			if (!grantsMatch) {
				// Only set grantsChanged if not in login progress
				if (!this.isLoginInProgress) {
					this.grantsChanged = true;
				}

				if (this.autoLogoutOnGrantChange) {
					throw new Error(
						"Grant configuration has changed. Auto-logout enabled.",
					);
				}
			}

			// If we reach here, grants are valid (either matching or auto-logout is disabled)
	...
	/**
	 * Get the current grants changed state.
	 *
	 * @returns {boolean} - Returns true if grants have changed since last authentication.
	 */
	getGrantsChanged(): boolean {
		return this.grantsChanged;
	}
