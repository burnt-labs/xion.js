...
export async function createEthWalletAccount(
  ...
  console.log(`${logPrefix} Creating account for Ethereum address:`, ethereumAddress);
  ...
  const messageHex = formatHexMessage(Buffer.from(calculatedAddress, 'utf8').toString('hex'));
  ...
  console.log(`${logPrefix} Account created successfully:`, result.account_address, `(tx: ${result.transaction_hash})`);
