 ...
 */
export function createAbstraxionBackend(config: AbstraxionBackendConfig): AbstraxionBackend {
  // Validate required configuration
  validateConfig(config);

  // Validate encryption key
  if (!EncryptionService.validateEncryptionKey(config.encryptionKey)) {
    throw new Error('Invalid encryption key format. Must be a base64-encoded 32-byte key.');
  }
...
function validateConfig(config: AbstraxionBackendConfig): void {
  ...
  if (config.refreshThresholdMs && config.sessionKeyExpiryMs && 
