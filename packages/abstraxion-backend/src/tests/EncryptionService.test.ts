...
describe('EncryptionService', () => {
  ...
  describe('constructor', () => {
    ...
    it('should throw error for empty master key', () => {
      ...
      expect(() => new EncryptionService('')).toThrow('Master encryption key is required');
    });
    ...
    it('should throw error for undefined master key', () => {
      expect(() => new EncryptionService(undefined as any)).toThrow(EncryptionError);
    });
  });
  ...
  describe('encryptSessionKey', () => {
    ...
    it('should handle long session key', async () => {
      ...
      const encrypted = await encryptionService.encryptSessionKey(longSessionKey);
      expect(encrypted).toBeDefined();
      expect(typeof encrypted).toBe('string');
    });
  ...
  describe('decryptSessionKey', () => {
    ...
    it('should throw error for invalid base64', async () => {
      await expect(encryptionService.decryptSessionKey('invalid-base64!')).rejects.toThrow(EncryptionError);
    });
    ...
    it('should throw error for corrupted data', async () => {
      ...
      await expect(encryptionService.decryptSessionKey(corrupted)).rejects.toThrow(EncryptionError);
    });

    it('should throw error for empty string', async () => {
      await expect(encryptionService.decryptSessionKey('')).rejects.toThrow(EncryptionError);
    });

    it('should throw error for too short data', async () => {
      const shortData = Buffer.from('short').toString('base64');
      await expect(encryptionService.decryptSessionKey(shortData)).rejects.toThrow(EncryptionError);
    });
  });
  ...
  describe('validateEncryptionKey', () => {
    ...
    it('should reject null and undefined', () => {
      ...
      expect(EncryptionService.validateEncryptionKey(undefined as any)).toBe(false);
    });
  });
  ...
  describe('encryption consistency', () => {
    ...
    it('should not decrypt with different master key', async () => {
      ...
      await expect(service2.decryptSessionKey(encrypted)).rejects.toThrow(EncryptionError);
    });
  });
  ...
  describe('security properties', () => {
    ...
    it('should detect tampering attempts', async () => {
      ...
      await expect(encryptionService.decryptSessionKey(tampered)).rejects.toThrow(EncryptionError);
    });
  });
});
