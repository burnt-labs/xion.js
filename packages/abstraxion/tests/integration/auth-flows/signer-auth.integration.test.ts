...
describe("Signer Authentication Flow - Integration", () => {
  ...
  describe("Secp256K1 Connector Flow", () => {
    it("should connect with Secp256K1 connector", async () => {
      ...
      expect(["idle", "connected", "connecting"]).toContain(initState.status);
      ...
      if (finalState.status === "connected") {
        ...
        expect(isValidXionAddress(finalState.account!.granterAddress!)).toBe(
          true,
        );
    ...
    it("should handle address formatting for EthWallet", async () => {
      const signerAuth: SignerAuthentication = {
        ...
        async getSignerConfig() {
          ...
          expect(result.authenticator).toBe(result.authenticator.toLowerCase());
  ...
  describe("Session Restoration", () => {
    ...
    it("should handle corrupted session data gracefully", async () => {
      ...
      await storageStrategy.setItem("abstraxion_session", "invalid-json-data");
